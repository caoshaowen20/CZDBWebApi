...
namespace CZDBHelper
{
  ...
  */
    ...
    public class DbSearcher: DbIpv4Searcher, DbIpv6Searcher
    {
         ...
         */
        private Cz88RandomAccessFile raf = null;
        ...
        public DbSearcher(String dbFile, QueryType queryType, String key)
        {
            this.queryType = queryType;

            // 这里的 fileStream 就是表示文件的 Stream
            // 可以对这个 Stream 进行进一步的操作

            HyperHeaderBlock headerBlock;

            using (FileStream fileStream = new FileStream(dbFile, FileMode.Open, FileAccess.Read))
            {
                headerBlock = HyperHeaderDecoder.decrypt(fileStream, key);
            }

            raf = new Cz88RandomAccessFile(dbFile, FileMode.Open, headerBlock.getHeaderSize());
        ...
        private void loadGeoSetting(Cz88RandomAccessFile raf, String key)
        {
            ...
            try
            {
                ...
                raf.Read(geoMapData, 0, geoMapData.Length);
        ...
        private void initializeForMemorySearch()
        {
        ...
        private void initMemoryOrBinaryModeParam(byte[] bytes, long fileSize)
        {
            ...
            {
                throw new Exception($"db file size error, excepted {fileSizeInFile}, real {fileSize}");
            }
            ...
            Array.Copy(bytes, DbConstant.SUPER_PART_LENGTH, b, 0, b.Length);
            initHeaderBlock(b);
        }

        private void initBtreeModeParam(Cz88RandomAccessFile raf)
        {
            // set db type
            raf.seek(0);
            byte[] superBytes = new byte[DbConstant.SUPER_PART_LENGTH];
            ...
            {
                throw new Exception($"db file size error, excepted {fileSizeInFile}, real {realFileSize}");
            }
        ...
        private void initHeaderBlock(byte[] headerBytes)
        {
            ...
            HeaderSip = new byte[len][];
            for (int i = 0; i < len; i++)
            {
                HeaderSip[i] = new byte[16];
            }
            ...
            for (int i = 0; i < headerBytes.Length; i += indexLength)
            {
                ...
                Array.Copy(headerBytes, i, HeaderSip[idx], 0, 16);
        ...
        private DataBlock memorySearch(byte[] ip)
        {
            ...
            while (l <= h)
            {
                ...
                Array.Copy(dbBinStr, p, sip, 0, ipBytesLength);
                Array.Copy(dbBinStr, p + ipBytesLength, eip, 0, ipBytesLength);
        ...
        private DataBlock bTreeSearch(byte[] ip)
        {
            int[] sptrNeptr = searchInHeader(ip);
            ...
            while (l <= h)
            {
                int m = (l + h) >> 1;
                int p = m * blen;
                Array.Copy(iBuffer, p, sip, 0, ipBytesLength);
                Array.Copy(iBuffer, p + ipBytesLength, eip, 0, ipBytesLength);
            ...
            raf.seek(dataPtr);
            byte[] region = new byte[dataLen];
            raf.Read(region, 0, region.Length);
            return new DataBlock(region, dataPtr);
        }
        ...
        {
            IPAddress inetAddress = IPAddress.Parse(ip);
            byte[] ipBytes = inetAddress.GetAddressBytes();
        ...
        private static int compareBytes(byte[] bytes1, byte[] bytes2, int length)
        {
            ...
            {
                return Comparer<int>.Default.Compare(bytes1.Length, bytes2.Length);
            }
        }
    ...
    public interface DbIpv4Searcher
    {
        String search(String ip);
    }

    public interface DbIpv6Searcher
    {
        String search(String ip);
    }
}